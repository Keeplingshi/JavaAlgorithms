package com.main.wangyiyoudao2017neitui;
...
public class Question1 {
	 ...
	 * 
	 * 变换一次的坐标变化：
	 * if(i<=n) 2*i-1 -> i		//即第i个值放在了第2*i-1的位置上
	 * else     (i-n)*2 -> i 	//即第i个值放在了第(i-n)*2的位置上
	 * 
	 * @param array
	 * @param k
	 */
	private static void getFinalSequence(int[] array,int n, int k) 
	{
		for(int j=0;j<k;j++)
		{
			...
			{
				if(i<=n){
					array[2*i-1]=tmpArr[i];
				}else{
					array[(i-n)*2]=tmpArr[i];
				}
			}
		}
