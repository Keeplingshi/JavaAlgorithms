...
//		lavista
		...
		while(sc.hasNext())
		{
			...
			int first=nmflag.indexOf(firststr);
			int second=nmflag.lastIndexOf(secondstr);
			if(first!=-1&&second!=-1)
			{
				if(first+firststr.length()<=second)
				{
					a=true;
				}
				
				first=nmflag.lastIndexOf(firststr);
				second=nmflag.indexOf(secondstr);
				if(second+secondstr.length()<=first)
				{
					b=true;
				}
